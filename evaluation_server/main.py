...
async def evaluation_engine(websocket: WebSocketServerProtocol):
    ...
    while True:
        # Wait for input
        raw_data: bytes = await websocket.recv()
        unpacked_data = tuple(i[0] for i in message_struct.iter_unpack(raw_data))
        print(f"<<< {raw_data} - {unpacked_data}")

        # Converge the model with new input values
        for log in model.convergence_iterator(unpacked_data, limit=50):
